...
uint16_t last_led_config,v12_v24,d1,s;
//uint16_t setLedStatus;
uint8_t i;
...
volatile uint16_t setColor = 0x0000;
volatile uint16_t setLedStatus = 0x0000;
...
void LEDdance ()
{
  for (setColor = 0 ; setColor < 0xFFFF ; setColor++)
  {
    setLedStatus = setColor;
    Delay (2000);
    RGB1_SETUP();
   }
...
//------usr code-----
  ...
  HAL_NVIC_EnableIRQ(EXTI2_3_IRQn);
   // update battery indicator LED value
        ...
        // Turn on device
        //setLedStatus = SWITCH1_BLUE + SWITCH2_RED + SWITCH3_BLUE + LBM_GREEN + STATUS_GREEN;
        RGB1_SETUP();
 ...
 //       
  while (1)
  {
   ...
   // else if ((powerOnButton_timer == 0) && (powerStatus == POWEROFF))
    ...
    {
     // SW2_Count = 1;  I dont understand why this code is here could this be the issue comment out and see no becasue then we cant turn on
    }
    ...
    else if (powerStatus == POWERON)      //----------DOUBLE BUTTONS:-----SW1 & SW2---------------------------
   {
      ...
      if((HAL_GPIO_ReadPin(SW1_GPIO_Port, SW1_Pin) == GPIO_PIN_RESET)&& (HAL_GPIO_ReadPin(SW2_GPIO_Port, SW2_Pin) == GPIO_PIN_SET) && 
        (HAL_GPIO_ReadPin(SW3_GPIO_Port, SW3_Pin) == GPIO_PIN_SET))
      {
        //setLedStatus = SWITCH1_GREEN + SWITCH2_RED + SWITCH3_BLUE + powerIndicatorStatus + STATUS_GREEN;
      ...
      //----------------SW3---------------------
      else if ((HAL_GPIO_ReadPin(SW3_GPIO_Port, SW3_Pin) == GPIO_PIN_RESET)&& (HAL_GPIO_ReadPin(SW2_GPIO_Port, SW2_Pin) == GPIO_PIN_SET) && 
        (HAL_GPIO_ReadPin(SW1_GPIO_Port, SW1_Pin) == GPIO_PIN_SET))
      {
        //setLedStatus = SWITCH1_BLUE + SWITCH2_RED + SWITCH3_GREEN + powerIndicatorStatus + STATUS_GREEN;
        //setLedStatus = 0x673D;
         setLedStatus = 0x0DC9;
        //LEDstop();
        RGB1_SETUP();
      ...
      else
      {
        ...
        {
          //setLedStatus = SWITCH1_BLUE + SWITCH2_RED + SWITCH3_BLUE + powerIndicatorStatus + STATUS_GREEN;
          setLedStatus = 0x0AF6;
          RGB1_SETUP();
          buttonWasActive = FALSE;
          backlight_timer = BACKLIGHTTIMERMS;
...
///* Private variables ---------------------------------------------------------*/
//
//uint16_t setLedStatus,last_led_config,v12_v24,d1,s,setColor;
//uint8_t i;
//
//volatile uint8_t SW2_Count = 0;
//volatile uint8_t SW2_Activated = 0;
...
////RGB1_SETUP();
//{
//  for (setColor = 1 ; setColor < 0xFFFF ; setColor++)
//  {
//    setLedStatus = setLedStatus >> setColor;
//    //Delay (1);
//    
//    //setColor++;
//  }
...
//    }
//    
//    else if (powerStatus == POWERON)      
//    {
//      
//      if ((HAL_GPIO_ReadPin(SW1_GPIO_Port, SW1_Pin) == GPIO_PIN_RESET)&&
//          (HAL_GPIO_ReadPin(SW2_GPIO_Port, SW2_Pin) == GPIO_PIN_RESET)&& 
//            (HAL_GPIO_ReadPin(SW3_GPIO_Port, SW3_Pin) == GPIO_PIN_SET)) // DONT FORGET SW3 CHECK FOR SET HERE
//      {
//        setLedStatus = SWITCH1_RED + SWITCH2_RED + SWITCH3_BLUE + powerIndicatorStatus + STATUS_GREEN;
//        
//        RGB1_SETUP();
//        Delay(1);
//        buttonWasActive = TRUE;
...
//      
//      //------------SW1---------------------------
//      else if((HAL_GPIO_ReadPin(SW1_GPIO_Port, SW1_Pin) == GPIO_PIN_RESET)&& (HAL_GPIO_ReadPin(SW2_GPIO_Port, SW2_Pin) == GPIO_PIN_SET) && 
//              (HAL_GPIO_ReadPin(SW3_GPIO_Port, SW3_Pin) == GPIO_PIN_SET))
//      {
//        setLedStatus = SWITCH1_GREEN + SWITCH2_RED + SWITCH3_BLUE + powerIndicatorStatus + STATUS_GREEN;
//        //Delay (1);
...
//    if (SW2_Count == SWITCHACTIVENO)//--------check number of times SW2 is pressed
//    {
//      if (powerStatus == POWEROFF)
//      {
//        // update battery indicator LED value
...
//        // Turn on device
//        setLedStatus = SWITCH1_BLUE + SWITCH2_RED + SWITCH3_BLUE + powerIndicatorStatus + STATUS_GREEN;
//        //setLedStatus = 0x0100 + 0x0011 + 0x0001 + 0x0010;
//        //setLedStatus= 0x4234;
//        //setLedStatus= 0x2509;
//        //setLedStatus= 0x88C2;
//        //setLedStatus= 0x00A6;
...
//        
//                        
//        //setLedStatus = SWITCH1_BLUE + SWITCH2_RED + SWITCH3_BLUE + LBM_AMBER + STATUS_GREEN;
//        //LEDdance();
//        RGB1_SETUP();
//        
//        backlight_timer = BACKLIGHTTIMERMS;
